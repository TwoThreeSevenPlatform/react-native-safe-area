...
lastReceivedSafeAreaInsets = { top: 0, bottom: 0, left: 0, right: 0 }
...
export function withSafeArea(
  ...
  return class extends React.Component<Props, State> {
    ...
    state = { safeAreaInsets: lastReceivedSafeAreaInsets }
    wrappedRef: any = null
    ...
    onSafeAreaInsetsDidChange(result: { safeAreaInsets: SafeAreaInsets }): void {
      ...
      lastReceivedSafeAreaInsets = safeAreaInsets
      if (!this.safeAreaInsetsEqualToCurrent(safeAreaInsets)) {
        this.setState({ safeAreaInsets })
      }
    }

    safeAreaInsetsEqualToCurrent(safeAreaInsets) {
      if (this.state.safeAreaInsets.top !== safeAreaInsets.top) {
        return false
      }
      if (this.state.safeAreaInsets.bottom !== safeAreaInsets.bottom) {
        return false
      }
      if (this.state.safeAreaInsets.left !== safeAreaInsets.left) {
        return false
      }
      if (this.state.safeAreaInsets.right !== safeAreaInsets.right) {
        return false
      }
      return true
    }
